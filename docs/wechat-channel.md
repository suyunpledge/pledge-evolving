...
> ⚠️ 如果你用 `forge setup` 生成配置：它只绑定 **medium** 这一档（因为只有该档的
> provider 行被写入了真实凭据）。`lite` / `premium` 保持 base.json 的占位环回地址，
> 因此在 `--strategy economy` 或 `premium` 下不会打到你的真实 API。
> 需要多档请手动补 provider 行并加进 `routing.tiers`。
...
### lane 缓存有上限（LRU，默认 128）

每个 lane 持有一个 agent（session + policy + 工具注册表），无上限的字典会随
bot 见过过的 peer 数一直增长。`_LaneCache` 是 LRU，默认 128 条，超出淘汰最久未用的
那条（可用 `--lane-cache N` 调整）。

淘汰是安全的：**lane 的持久状态是磁盘上的 session 文件，不是内存里的 agent**。
重建时会重新打开同一个日志文件。
...
同一账号起两个 `serve` 会互相抢游标并双重回复。用锁文件（放在游标旁边）拒绝第二个进程。

判活条件是**「PID 存在 **且** 锁在持续刷新」**，这一对组合解决了只看 PID 的三种误判：

| 误判 | 只看 PID 的后果 | 加上心跳后 |
|---|---|---|
| **PID 回收**（崩溃后新进程复用同一 PID） | 新 PID 活着 → 锁永远不可回收 | 新进程没在刷新**这个**锁 → 锁陈旧 → 可接管 |
| **跨用户 PID**（`os.kill(pid,0)` 抛 PermissionError） | 保守判活 → 锁永远不可回收 | 陈旧判定兜底放行 |
| **无法探测 PID**（无 tasklist / 沙箱环境） | 默认判活 → 锁死 | 退化为纯陈旧判定，不假设活着 |

持锁进程每轮轮询刷新一次锁的 mtime（轮询窗口 ≤60s，陈旧阈值 180s）。
锁文件损坏按陈旧处理而非致命。
...
- **单进程**。同一账号起两个 `serve` 会被锁文件拦住（不会静默双跑）；锁靠心跳判活，
  进程被 `kill -9` 后最多 180 秒他人才能接管。
- **lane 缓存上限 128**（LRU）。超出后最久未用的会话 agent 会被丢弃重建（会话日志不丢）。
